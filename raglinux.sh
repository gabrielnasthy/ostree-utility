...
function ENV_CREATE_OPTS {
    ...
    declare -g OSTREE_SYS_KARG=${OSTREE_SYS_KARG:='rootflags=subvol=@ rootfstype=btrfs'}
    declare -g OSTREE_SYS_BOOT_LABEL=${OSTREE_SYS_BOOT_LABEL:='ARCHISO_EFI'}
    declare -g OSTREE_SYS_ROOT_LABEL=${OSTREE_SYS_ROOT_LABEL:='RAGLinux'}
    declare -g OSTREE_OPT_NOMERGE=${OSTREE_OPT_NOMERGE='--no-merge'}
    declare -g OSTREE_REP_NAME=${OSTREE_REP_NAME:='raglinux'}
    ...
    declare -g SYSTEM_OPT_TIMEZONE=${SYSTEM_OPT_TIMEZONE:='America/Cuiaba'}
    declare -g SYSTEM_OPT_KEYMAP=${SYSTEM_OPT_KEYMAP:='br-abnt2'}

    declare -g PODMAN_OPT_BUILDFILE=${PODMAN_OPT_BUILDFILE:="${0%/*}/Containerfile.base:ostree/base"}
...
# [DISK]: PARTITIONING (GPT+UEFI) - SKIP AS PARTITIONS ALREADY EXIST
function DISK_CREATE_LAYOUT {
    echo "Partições já existem, pulando criação..."
    return 0
}

# [DISK]: FILESYSTEM (ESP+BTRFS) - SKIP AS FILESYSTEMS ALREADY EXIST
function DISK_CREATE_FORMAT {
    echo "Sistemas de arquivo já existem, pulando formatação..."
    return 0
}
...
function DISK_CREATE_MOUNTS {
    ...
    # Montar partição root temporariamente para criar subvolumes
    mount ${OSTREE_DEV_ROOT} ${OSTREE_SYS_ROOT}
    
    # Criar subvolumes BTRFS
    btrfs subvolume create ${OSTREE_SYS_ROOT}/@ || true
    btrfs subvolume create ${OSTREE_SYS_ROOT}/@home || true
    btrfs subvolume create ${OSTREE_SYS_ROOT}/@var || true
    btrfs subvolume create ${OSTREE_SYS_ROOT}/@ostree || true
    
    # Desmontar e remontar com subvolumes
    umount ${OSTREE_SYS_ROOT}
    mount -o subvol=@ ${OSTREE_DEV_ROOT} ${OSTREE_SYS_ROOT}
    
    # Criar diretórios e montar subvolumes
    mkdir -p ${OSTREE_SYS_ROOT}/{home,var,ostree}
    mount -o subvol=@home ${OSTREE_DEV_ROOT} ${OSTREE_SYS_ROOT}/home
    mount -o subvol=@var ${OSTREE_DEV_ROOT} ${OSTREE_SYS_ROOT}/var
    mount -o subvol=@ostree ${OSTREE_DEV_ROOT} ${OSTREE_SYS_ROOT}/ostree
    
    # Montar partição EFI
    mkdir -p ${OSTREE_SYS_ROOT}/boot/efi
    mount ${OSTREE_DEV_BOOT} ${OSTREE_SYS_ROOT}/boot/efi
}
            ...
            case ${CLI_VAL} in
                ...
                * | 'help')
                    local USAGE=(
                        ...
                        'Options:'
                        '  -b, --base-os string      : (install/upgrade) : Name of OS to use as a base. Defaults to raglinux'
